...
def plongement(x: list, d: int, time_delay: int):
    n = len(x)
    m = (d-1)*time_delay
    ...
    for i in range(m, n):
        xn = []
        for j in range(d):
            xn.append(x[i - (d-j-1)*time_delay])
        xn.reverse()
        y.append(xn)
...
def animation_plongement_2d(x, tau_pas, tau_max):
    fig, ax = plt.subplots()
    Xs = []
    for tau in range(0, tau_max, tau_pas):
        Xs.append(plongement(x, d=2, time_delay=tau))
    ...
    ani = animation.FuncAnimation(fig, animate, range(int(tau_max/tau_pas)), blit=True, init_func=init, interval=500, repeat=False)
    plt.show()


def plot_plongement_2d(fig, ax, x: list, time_delay: int):
    ax.set_xlabel("x")
    ax.set_ylabel("y")
